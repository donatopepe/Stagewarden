...
- `/role add-child <parent_node> <role_type> [node_id]` aggiunge nodi PRINCE2 delegati/subordinati alla baseline approvata.
- `/role assign <node_id> <provider> <provider_model> [reasoning_effort=<valore>] [account=<nome>]` assegna un provider-model a un nodo specifico dell'albero.

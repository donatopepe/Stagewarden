...
- Roadmap research follow-up (2026-05-31): `ROADMAP.md` now defines a staged evolution plan grounded in internal command/schema and gating gaps plus OpenTelemetry GenAI, Anthropic agent workflows, NIST AI RMF, and OWASP GenAI 2025. First recommended implementation slice is Agent Run Ledger with OpenTelemetry-compatible export.
...
- PRINCE2/coding-agent evolution (2026-05-30): studied local PRINCE2 material in `/Users/donato/study` plus institutional PRINCE2 references; implemented first high-impact tranche by injecting `Coding work package controls` into executor prompt packets. The controls force product focus, expected output, acceptance criteria, quality gates, TDD/failing-test discipline, minimal implementation, focused wet-run evidence, scope control, and exception escalation.
- Validation for this tranche: the new executor prompt test was written first and observed failing (`AssertionError: 'product focus' not found in ''`), then passed after implementation. Focused validation passed: `python3.11 -m py_compile stagewarden/executor.py stagewarden/executor_prompting.py tests/test_executor.py`, `python3.11 -m unittest tests.test_executor -v` (`50 OK`), and `python3.11 -m unittest tests.test_rag -v` (`15 OK`). Full `python3.11 -m unittest discover -s tests -v` was attempted but timed out at 600s after three missing-OpenRouter-key failures in live/provider tests; focused rerun confirmed those fail at `_openrouter_env_name()` before exercising product code.
- PRINCE2 enforcement slice (2026-05-30): explicit coding/test work-package completions now require prior successful same-step non-model tool transcript evidence. RED was observed on narrative-only `complete` claiming `unittest`/`exit_code=0`; after implementation, reject/accept evidence tests pass, `py_compile` passes, and `python3 -m unittest tests.test_executor -v` reports `52 OK`.
- PRINCE2 product checkpoint slice (2026-05-30): completed coding/product-mutating work packages now persist `product_checkpoint` handoff entries with product description, acceptance criteria, quality-gate evidence, checkpoint status, model/action metadata, and product id. RED was observed in the stub-backend agent integration test because no checkpoint existed; focused GREEN validation passed with 57 tests OK across non-live agent integration paths plus `tests.test_executor`.
- PRINCE2 checkpoint recovery slice (2026-05-30): resumed runs and live runs whose latest observation shows a failed completion/quality gate now use the latest `product_checkpoint` to inject a ready `checkpoint-recovery-step-*` before retrying the blocked work package. The generated step carries failed gate text, product description, acceptance criteria, and prior quality evidence so the next action captures missing real non-model evidence instead of repeating the same closure prompt. RED/GREEN validation passed in `tests.test_planner` and `tests.test_agent_integration`, py_compile passed, and focused impacted suites report 65 OK.
- PRINCE2 non-coding product-evidence slice (2026-05-30): wet-run-required design/documentation/specification artifact closures now require prior successful same-step non-model tool evidence before `complete`, extending the earlier coding-only evidence gate without matching generic planning/routing terms. RED showed a narrative-only ADR/design closure was accepted; GREEN reject/accept design tests passed, and `python3 -m py_compile stagewarden/executor.py tests/test_executor.py && python3 -m unittest tests.test_executor tests.test_planner -v` reported 62 OK.
...
- Validation follow-up (2026-05-24): cross-cutting post-hardening regression passed (`python3 -m unittest tests.test_rag tests.test_executor tests.test_json_schema_registry -v` -> `66 OK`).
- Follow-up fix (2026-05-24): OpenRouter benchmark retry path in `tests/test_trace_cli.py` now conditionally asserts `history.previous` (null on first-attempt success, non-null after retry). Focused validation passed: `python3 -m unittest tests.test_trace_cli.TraceAndCliTests.test_openrouter_benchmark_cli_reports_multi_suite_baseline tests.test_trace_cli.TraceAndCliTests.test_openrouter_benchmark_history_comparison_detects_regressions -v`.
- Full-suite follow-up (2026-05-24): `python3 -m unittest discover -s tests -v` passed (`431 OK`) after adding live-model quality-drift skip handling to the OpenRouter benchmark trace test.
- Maintenance (2026-05-24): cleaned duplicated repeated validation bullets in `AGENT_HANDOFF.md` to keep handoff concise and navigable.
...
- `role message --json` now exposes top-level `rag_indexed` and `rag_entry_id` telemetry for communication-RAG indexing observability.
- `roles tick --json` now exposes top-level `rag_context_by_node` for batch machine-readable RAG communication context.
- non-JSON `roles tick` output now includes `rag_context_nodes` count in the batch summary line.
- trace CLI regression now verifies `rag_context_nodes` is present in non-JSON `roles tick` output.
- `roles tick` now emits deterministic `rag_context_by_node` directly from batch runtime results, enabling strict node-level assertions in JSON tests.
- non-JSON `roles tick` summary now also includes `rag_context_node_ids` for quick per-node visibility.
- `roles tick --json` now includes `rag_context_summary` (`count`, `node_ids`) for compact machine-readable aggregation.
- role-tick lifecycle JSON test now handles legitimate `rag_context=None` on post-consumption ticks while preserving typed validation when context is present.
- role-message JSON test now verifies consistency between `rag_indexed` and `rag_entry_id`.
- roles-tick JSON test now verifies `rag_context_summary.node_ids` is consistent with `rag_context_by_node` keys.
- Previously planned research/report/plan evidence gates are now complete; checkpoint-recovery status reporting remains optional and should only be started if a concrete operator-visible gap is identified.
...
- Stale-baseline supervision views are now informational rather than blocking: `roles active`, `roles queues`, `roles control`, and `roles messages` keep exit 0 while surfacing stale-baseline warning/context in text and JSON. Full discovery validation after this slice passed: `set -a; . ~/.hermes/.env; set +a; python3.11 -m unittest discover -s tests -q` -> 445 tests OK in 1167.000s.
- Non-coding evidence enforcement follow-up is complete: research/report/plan file artifacts now require same-step non-model tool evidence before `complete`, while generic planning/review steps without explicit artifact references remain non-blocking. Focused RED/GREEN and executor/planner/agent-integration regression are green, and full discovery passed with explicit Hermes env sourcing: `set -a; . ~/.hermes/.env; set +a; python3.11 -m unittest discover -s tests -q` -> 449 tests OK in 1136.235s.
...
- The repository is on branch `main`; the validated research/report/plan evidence-gate follow-up is committed in current HEAD, and the roadmap research follow-up is documented in `ROADMAP.md` and committed in current HEAD.
...
- Checkpoint-recovery status reporting slice (2026-05-30): `stagewarden/project_handoff_views.py` now exposes `checkpoint_recovery` in `stage_view()` with active/pending/completed counts, active recovery backlog items, and latest `product_checkpoint` metadata; rendered stage views now print a compact `checkpoint_recovery:` line for operator visibility. RED was observed in `tests.test_trace_cli.TraceAndCliTests.test_stage_view_reports_checkpoint_recovery_status` with `KeyError: 'checkpoint_recovery'`; GREEN focused validation passed with py_compile and the new checkpoint-recovery stage-view regression plus handoff-actions regression.
- Ambiguous-brief clarification slice (2026-05-30): `stagewarden/project/brief.py` now detects placeholder values such as `TBD`, `unknown`, `to be decided`, and Italian equivalents in required project-brief fields; project design, tree proposal, and project start now treat those values as `ambiguous_<field>` clarification gaps instead of approving a plan from assumptions. RED was observed in `tests.test_trace_cli.TraceAndCliTests.test_project_start_requests_clarification_for_ambiguous_brief_values` because `objective=TBD` still approved startup; GREEN focused validation passed with py_compile, project-start/tree clarification regressions, persistence/PRINCE2 coverage, and ready-project startup regressions. Full regression validation also passed after sourcing Hermes env explicitly: `set -a; . ~/.hermes/.env; set +a; python3.11 -m unittest discover -s tests -q` -> 440 OK in 1064.353s.
- Project-brief guidance ambiguity slice (2026-05-30): `project brief` and `project brief set` now prioritize ambiguous required values over later missing fields, returning JSON `ambiguous_gaps` and `next_missing_gap` so automation can repair the exact placeholder field. RED: python3.11 -m unittest tests.test_trace_cli.TraceAndCliTests.test_project_brief_set_reports_ambiguous_field_before_missing_fields -v failed because project brief guidance skipped ambiguous objective=TBD and pointed to missing scope; GREEN: focused ambiguous guidance test passed; py_compile plus 4 brief/start/design tests passed; 5 project-tree approval/proposal regressions passed.

...
- The smoke script now performs a live OpenRouter API call through `RUN_MODEL_BIN` and no longer relies on ChatGPT/device-login simulation or secret-store placeholders.
...
- **OpenRouter Smoke Script:** The `scripts/test_chatgpt_flow.sh` smoke path now performs a live OpenRouter API call and no longer depends on ChatGPT login or secret-store simulation.
...
- **External IO Fallback Preservation:** The JSON fallback for `external_io` now keeps the exact input command string in the payload and uses a dedicated shared schema.
...
- The backend-injection tests now exercise the real OpenRouter API key from the environment for transport coverage rather than simulated account tokens.
...
- The devil's-advocate critic path now rejects invalid review payloads explicitly, and the battery/integration stubs were updated to emit valid critic JSON during wet-run simulations.

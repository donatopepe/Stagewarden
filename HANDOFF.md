...
- Deep trace follow-up fixed refactor fallout: browser/system/watch/external IO CLI routing, slash fuzzy highlighting, main.py compatibility patch targets for auth/catalog/runtime, provider auth metadata in `model list`, provider-limit summary scope, PRINCE2 role defaults/rendering, roles flow/tick owner routing, and project-tree AI design import. Full `tests.test_trace_cli` reached 200 tests with 4 remaining failures; those 4 were fixed and revalidated in focused tests, but the full module was not rerun after the final fixes due runtime cost.

...
from . import command_dispatch as _command_dispatch
...
def run_cli() -> int:
    ...
    if args.openrouter_benchmark:
        report = _openrouter_benchmark.run_openrouter_benchmark(history_path=args.openrouter_benchmark_history)
        rendered = dumps_ascii(_json_schema_registry.with_json_schema("openrouter benchmark", report), indent=2)
    ...
    if args.prince2_benchmark:
        report = _prince2_benchmark.run_prince2_benchmark()
        rendered = dumps_ascii(_json_schema_registry.with_json_schema("prince2 benchmark", report), indent=2)
    ...
    if args.caveman_help:
        ...
        return _shell_views._run_interactive_shell_impl(config)
    if task in {"help", "help topics", "help --json", "help topics --json"}:
        if args.json or task.endswith("--json"):
            print(dumps_ascii(_json_schema_registry.with_json_schema("help", _ui_views._help_json_report()), indent=2))
        else:
            print(_ui_views.interactive_help_text())
        return 0
    if task.startswith("help "):
        ...
        if topic.endswith(" --json"):
            ...
            if raw_topic.lower() == "caveman":
                print(dumps_ascii(_json_schema_registry.with_json_schema("help", {"command": "help", "ok": True, "topic": "caveman", "message": "Use `help caveman` for the rich caveman help surface."}), indent=2))
            else:
                print(dumps_ascii(_json_schema_registry.with_json_schema("help", _ui_views._help_json_report(raw_topic)), indent=2))
            return 0
        if args.json:
            print(dumps_ascii(_json_schema_registry.with_json_schema("help", _ui_views._help_json_report(topic)), indent=2))
        elif topic.lower() == "caveman":
            print(Agent(config=config).caveman.help_text())
        elif topic.lower() == "topics":
            print(_ui_views.interactive_help_text())
        else:
            print(_ui_views.interactive_help_text(topic))
        return 0
    if task in {"commands", "commands --json"}:
        if args.json or task == "commands --json":
            print(dumps_ascii(_json_schema_registry.with_json_schema("commands", {"command": "commands", "commands": _commands.command_catalog()}), indent=2))
        else:
            print(_commands.render_command_catalog())
        return 0
    if task == "slash choose" or task.startswith("slash choose "):
        query = "" if task == "slash choose" else task.split(maxsplit=2)[2]
        if args.json:
            report = _ui_views._slash_palette_report(config, query)
            print(
                dumps_ascii(
                    _json_schema_registry.with_json_schema(
                        "slash choose",
            ...
            print(_ui_views._render_slash_choice_candidates(config, query))
        return 0
    if task == "slash" or task.startswith("slash "):
        ...
        if args.json or task.endswith(" --json"):
            print(dumps_ascii(_json_schema_registry.with_json_schema("slash", _ui_views._slash_palette_report(config, prefix)), indent=2))
        else:
    ...
    if task == "doctor":
        ...
        if args.json:
            print(dumps_ascii(_json_schema_registry.with_json_schema("doctor", report), indent=2))
        else:
            print(rendered)
    ...
    if task == "status":
        ...
        if args.json:
            print(dumps_ascii(_json_schema_registry.with_json_schema("status", _status_views._status_dashboard_report(agent, config) if args.full else _status_views._status_report(agent, config)), indent=2))
        else:
    ...
    if task in {"status full", "status --full"}:
        agent = _agent_setup_views._configure_readonly_agent_for_workspace(config)
        if args.json:
            print(dumps_ascii(_json_schema_registry.with_json_schema("status", _status_views._status_dashboard_report(agent, config)), indent=2))
        else:
    ...
    if task == "statusline":
        agent = _agent_setup_views._configure_readonly_agent_for_workspace(config)
        print(dumps_ascii(_json_schema_registry.with_json_schema("statusline", _status_dashboard_views._statusline_report(agent, config)), indent=2))
        return 0
    if task == "baseline":
        if args.json:
            print(dumps_ascii(_json_schema_registry.with_json_schema("baseline", _status_views._agent_baseline_report(config)), indent=2))
        else:
            print(_status_views._render_agent_baseline(config))
        return 0
    if task == "battery":
        if args.json:
            print(dumps_ascii(_json_schema_registry.with_json_schema("battery", _battery_views._battery_report(config)), indent=2))
        else:
    ...
    if task == "preflight":
        agent = _agent_setup_views._configure_readonly_agent_for_workspace(config)
        if args.json:
            print(dumps_ascii(_json_schema_registry.with_json_schema("preflight", _status_dashboard_views._preflight_report(agent, config)), indent=2))
        else:
            print(_status_dashboard_views._render_preflight(agent, config))
        return 0
    if task == "shell backend":
        if args.json:
            print(dumps_ascii(_json_schema_registry.with_json_schema("shell backend", _shell_views._shell_backend_report(config)), indent=2))
        else:
            print(_shell_views._render_shell_backend(config))
        return 0
    if task.startswith("shell backend use "):
        response = _command_views._handle_shell_command(task.split(), config)
        payload = _json_schema_registry.with_json_schema("shell backend use", {"command": "shell backend use", "message": response, "report": _shell_views._shell_backend_report(config)})
        if args.json:
            print(dumps_ascii(_json_schema_registry.with_json_schema("shell backend use", payload), indent=2))
        else:
            print(response)
        return 0
    if task.startswith("auth status "):
        ...
        if args.json:
            print(dumps_ascii(_json_schema_registry.with_json_schema("auth status", _auth_views._auth_status_report(provider)), indent=2))
        else:
            print(_auth_views._render_auth_status(provider))
        return 0
    if task == "overview":
        agent = _agent_setup_views._configure_agent_for_workspace(config)
        if args.json:
            print(dumps_ascii(_json_schema_registry.with_json_schema("overview", _status_views._overview_report(agent, config)), indent=2))
        else:
    ...
    if task == "health":
        agent = _agent_setup_views._configure_agent_for_workspace(config)
        if args.json:
            print(dumps_ascii(_json_schema_registry.with_json_schema("health", _status_views._health_report(agent, config)), indent=2))
        else:
    ...
    if task == "report":
        agent = _agent_setup_views._configure_agent_for_workspace(config)
        if args.json:
            print(dumps_ascii(_json_schema_registry.with_json_schema("report", _status_dashboard_views._report_report(agent, config)), indent=2))
        else:
    ...
    if task == "models":
        agent = _agent_setup_views._configure_agent_for_workspace(config)
        if args.json:
            print(dumps_ascii(_json_schema_registry.with_json_schema("models", _status_views._model_status_report(agent, config)), indent=2))
        else:
    ...
    if task in {"model limits", "models limits"}:
        agent = _agent_setup_views._configure_readonly_agent_for_workspace(config)
        if args.json:
            print(dumps_ascii(_json_schema_registry.with_json_schema("model limits", _status_views._model_limits_report(agent, config)), indent=2))
        else:
            print(_status_views._render_model_limits(agent, config))
        return 0
    if (
        ...
        agent = _agent_setup_views._configure_readonly_agent_for_workspace(config)
        response = _model_views._handle_model_command(task, agent, config)
        if response is None:
            response = _account_views._handle_account_command(task, agent, config)
        schema_command = " ".join(task.split()[:2])
        payload = _json_schema_registry.with_json_schema(schema_command, {"command": schema_command, "message": response})
        if args.json:
    ...
    if task == "catalog" or task.startswith("catalog "):
        ...
        if args.json:
            ...
            if len(parts) == 1 or parts[1] == "status":
                print(dumps_ascii(_json_schema_registry.with_json_schema("catalog status", _model_views._catalog_status_report()), indent=2))
                return 0
            if parts[1] == "search":
                if len(parts) < 3:
                    print(dumps_ascii(_json_schema_registry.with_json_schema("catalog search", {"command": "catalog search", "ok": False, "error": _model_views._catalog_usage()}), indent=2))
                    return 1
                ...
                if not query and not provider and not feature:
                    print(dumps_ascii(_json_schema_registry.with_json_schema("catalog search", {"command": "catalog search", "ok": False, "error": _model_views._catalog_usage()}), indent=2))
                    return 1
                print(dumps_ascii(_json_schema_registry.with_json_schema("catalog search", _model_views._catalog_search_report(query, provider, feature=feature)), indent=2))
                return 0
            if parts[1] == "refresh":
                try:
                    include_artificial_analysis = _model_views._parse_catalog_refresh_flags(parts[2:])
                except ValueError:
                ...
                return 0
        response = _model_views._handle_model_command(task, agent, config)
        payload = _json_schema_registry.with_json_schema("catalog", {"command": task, "message": response})
        if args.json:
            print(dumps_ascii(payload, indent=2))
        else:
            print(response or _model_views._catalog_usage())
        return 0 if response else 1
    if task == "model inspect" or task.startswith("model inspect "):
        agent = _agent_setup_views._configure_readonly_agent_for_workspace(config)
        parts = task.split()
        if len(parts) not in {3, 4}:
            payload = _json_schema_registry.with_json_schema("model inspect", {"command": "model inspect", "ok": False, "error": "Usage: model inspect <provider> [provider_model]"})
            if args.json:
                print(dumps_ascii(_json_schema_registry.with_json_schema("model inspect", payload), indent=2))
            else:
                print(payload["error"])
            return 1
        provider = parts[2]
        ...
        try:
            report = _model_inspection_views._inspect_provider_models(agent, config, provider=provider, provider_model=provider_model)
        except ValueError as exc:
            payload = _json_schema_registry.with_json_schema("model inspect", {"command": "model inspect", "ok": False, "error": str(exc)})
            if args.json:
                print(dumps_ascii(_json_schema_registry.with_json_schema("model inspect", payload), indent=2))
            else:
                print(payload["error"])
            return 1
        if args.json:
            print(dumps_ascii(_json_schema_registry.with_json_schema("model inspect", report), indent=2))
        else:
    ...
    if task.startswith("model "):
        ...
        response = _model_views._handle_model_command(task, agent, config)
        if response is None:
            print(_model_views._model_usage())
            return 1
        if args.json:
            print(dumps_ascii(_json_schema_registry.with_json_schema("model", {"command": task, "message": response, "models": _status_views._model_status_report(agent, config)}), indent=2))
        else:
            print(response)
        return 0
    if task == "accounts":
        if args.json:
            print(dumps_ascii(_json_schema_registry.with_json_schema("accounts", _account_views._accounts_report(config)), indent=2))
        else:
            print(_account_views._render_accounts(config))
        return 0
    if task == "roles":
        if args.json:
            print(dumps_ascii(_json_schema_registry.with_json_schema("roles", _project_role_views._prince2_roles_report(config)), indent=2))
        else:
            print(_project_role_views._render_prince2_roles(config))
        return 0
    if task == "project brief":
        if args.json:
            print(dumps_ascii(_json_schema_registry.with_json_schema("project brief", _project_flow._project_brief_report(config)), indent=2))
        else:
            print(_project_flow.render_project_brief(config))
        return 0
    if task == "project design":
        agent = _agent_setup_views._configure_readonly_agent_for_workspace(config)
        if args.json:
            print(dumps_ascii(_json_schema_registry.with_json_schema("project design", _project_design_flow._project_design_report(agent, config)), indent=2))
        else:
            print(_project_design_flow._render_project_design(agent, config))
        return 0
    if task in {"project tree propose", "project tree propose --ai"}:
        ...
        if args.json:
            print(dumps_ascii(_json_schema_registry.with_json_schema("project tree propose", report), indent=2))
        else:
    ...
    if task in {"project tree approve", "project tree approve --force"}:
        ...
        if args.json:
            print(dumps_ascii(_json_schema_registry.with_json_schema("project tree approve", report), indent=2))
        else:
    ...
    if task == "roles domains":
        if args.json:
            print(dumps_ascii(_json_schema_registry.with_json_schema("roles domains", _project_role_tree_views._prince2_role_domains_report()), indent=2))
        else:
            print(_project_role_tree_views._render_prince2_role_domains())
        return 0
    if task == "roles tree":
        if args.json:
            print(dumps_ascii(_json_schema_registry.with_json_schema("roles tree", _project_role_tree_views._prince2_role_tree_report(config)), indent=2))
        else:
            print(_project_role_tree_views._render_prince2_role_tree(config))
        return 0
    if task == "roles tree approve":
        agent = _agent_setup_views._configure_readonly_agent_for_workspace(config)
        response = _project_role_command_flow._handle_project_and_roles_command(task, agent, config)
        if response is None:
            response = _project_role_command_flow._handle_role_command(task, agent, config)
        if args.json:
            print(dumps_ascii(_json_schema_registry.with_json_schema("roles tree approve", _project_role_tree_views._prince2_role_tree_baseline_report(config)), indent=2))
        else:
            print(response)
        return 0
    if task == "roles baseline":
        if args.json:
            print(dumps_ascii(_json_schema_registry.with_json_schema("roles baseline", _project_role_tree_views._prince2_role_tree_baseline_report(config)), indent=2))
        else:
            print(_project_role_tree_views._render_prince2_role_tree_baseline(config))
        return 0
    if task == "roles baseline matrix":
        if args.json:
            print(dumps_ascii(_json_schema_registry.with_json_schema("roles baseline matrix", _project_role_tree_views._prince2_role_tree_baseline_matrix_report(config)), indent=2))
        else:
            print(_project_role_tree_views._render_prince2_role_tree_baseline_matrix(config))
        return 0
    if task.startswith("roles context "):
        node_id = task.split(maxsplit=2)[2]
        if args.json:
            print(dumps_ascii(_json_schema_registry.with_json_schema("roles context", _project_role_views._prince2_role_context_report(config, node_id)), indent=2))
        else:
            print(_project_role_views._render_prince2_role_context(config, node_id))
        return 0
    if task == "roles active":
        if args.json:
            print(dumps_ascii(_json_schema_registry.with_json_schema("roles active", _project_role_runtime_views._prince2_role_active_report(config)), indent=2))
        else:
    ...
    if task == "roles control":
        if args.json:
            print(dumps_ascii(_json_schema_registry.with_json_schema("roles control", _project_role_runtime_views._prince2_role_control_report(config)), indent=2))
        else:
            print(_project_role_runtime_views._render_prince2_role_control(config))
        return 0
    if task == "roles queues":
        if args.json:
            print(dumps_ascii(_json_schema_registry.with_json_schema("roles queues", _project_role_runtime_views._prince2_role_queue_report(config)), indent=2))
        else:
            print(_project_role_runtime_views._render_prince2_role_queues(config))
        return 0
    if task == "roles messages" or task.startswith("roles messages "):
        ...
        if args.json:
            print(dumps_ascii(_json_schema_registry.with_json_schema("roles messages", _project_role_runtime_views._prince2_role_messages_report(config, node_id=node_id)), indent=2))
        else:
    ...
    if task == "roles tick" or task.startswith("roles tick "):
        ...
        if task != "roles tick":
            try:
                ...
                error_payload = _json_schema_registry.with_json_schema("roles tick", {"command": "roles tick", "ok": False, "error": "Usage: roles tick [max_nodes]"})
                if args.json:
                    print(dumps_ascii(_json_schema_registry.with_json_schema("roles tick", error_payload), indent=2))
                else:
        ...
        if args.json:
            ...
            rag_context_node_ids = ",".join(sorted(key for key, value in rag_context_by_node.items() if key and isinstance(value, dict)))
            print(
                ...
                f"rag_context_nodes={rag_context_nodes} rag_context_node_ids={rag_context_node_ids or 'none'}.\n"
                + _project_role_runtime_views._render_prince2_role_runtime(config)
            )
        return 0
    if task == "roles check":
        if args.json:
            print(dumps_ascii(_json_schema_registry.with_json_schema("roles check", _project_role_tree_views._prince2_role_check_report(config)), indent=2))
        else:
            print(_project_role_tree_views._render_prince2_role_check(config))
        return 0
    if task == "roles flow":
        if args.json:
    ...
    if task == "roles matrix":
        if args.json:
            print(dumps_ascii(_json_schema_registry.with_json_schema("roles matrix", _project_role_tree_views._prince2_role_matrix_report(config)), indent=2))
        else:
            print(_project_role_tree_views._render_prince2_role_matrix(config))
        return 0
    if task.startswith("project brief "):
        response = _project_flow._handle_project_brief_command(task, config)
        if response is None:
        ...
        if args.json:
            ...
            print(dumps_ascii(_json_schema_registry.with_json_schema(schema_command, {"command": task, "message": response, "project_brief": _project_flow._project_brief_report(config)}), indent=2))
        else:
            print(response)
        return 0
    if task in {"project start", "project start --ai"}:
        ...
        report = _project_start_flow._project_start_report(agent, config, prefs, force_ai=task.endswith("--ai"))
        if args.json:
            print(dumps_ascii(_json_schema_registry.with_json_schema("project start", report), indent=2))
        else:
    ...
    if task in {"sources", "sources status"} or task.startswith("sources "):
        if args.json:
            if task == "sources update":
                report = _json_schema_registry.with_json_schema("sources update", _status_views._sources_update_report(config))
                print(dumps_ascii(_json_schema_registry.with_json_schema("sources update", report), indent=2))
                return 0 if report.get("ok") else 1
            if task in {"sources", "sources status", "sources status --strict"}:
                strict = task == "sources status --strict"
                report = _json_schema_registry.with_json_schema("sources status", _status_views._sources_status_report(config, strict=strict))
                print(dumps_ascii(_json_schema_registry.with_json_schema("sources status", report), indent=2))
                return 0 if not strict or report.get("ok") else 1
            print(dumps_ascii(_json_schema_registry.with_json_schema("sources status", {"command": task, "ok": False, "error": "Usage: sources | sources status [--strict] | sources update"}), indent=2))
            return 1
        response = _status_views._handle_sources_command(task, config)
        if response is None or response.startswith("Usage:"):
            print(response or "Usage: sources | sources status [--strict] | sources update")
            return 1
        print(response)
        return 0 if task != "sources status --strict" or _status_views._sources_status_report(config, strict=True).get("ok") else 1
    if task in {"update status", "update check", "update check --json", "update apply", "update apply --yes"} or task.startswith("update "):
        if args.json or task == "update check --json":
            if task in {"update status"}:
                report = _json_schema_registry.with_json_schema("update status", _status_views._update_status_report(config))
            elif task in {"update check", "update check --json"}:
                report = _json_schema_registry.with_json_schema("update check", _status_views._update_status_report(config, fetch=True))
            elif task in {"update apply", "update apply --yes"}:
                report = _json_schema_registry.with_json_schema("update apply", _status_views._update_apply_report(config, confirmed=task.endswith(" --yes")))
            else:
                report = _json_schema_registry.with_json_schema("update", {"command": task, "ok": False, "error": "Usage: update status | update check [--json] | update apply --yes"})
            print(dumps_ascii(_json_schema_registry.with_json_schema((report or {}).get("command", "update"), report), indent=2))
            return 0 if report.get("ok") else 1
        response = _status_views._handle_update_command(task, config)
        if response is None or response.startswith("Usage:"):
    ...
    if task == "extensions" or task.startswith("extension ") or task.startswith("extensions "):
        if args.json:
            if task == "extensions":
                report = _json_schema_registry.with_json_schema("extensions", _extension_views.discover_extensions(config.workspace_root))
            elif task.startswith("extension scaffold "):
                try:
                    report = _json_schema_registry.with_json_schema("extensions", _extension_views.scaffold_extension(config.workspace_root, task.split(maxsplit=2)[2]))
            ...
            print(dumps_ascii(_json_schema_registry.with_json_schema("extensions", report), indent=2))
            if task == "extensions":
                return 0
            return 0 if report.get("ok") else 1
        response = _extension_views._handle_extension_command(task, config)
    ...
    if task.startswith("file "):
        report = _command_views._file_command_report(task, config)
        if args.json:
            ...
            if schema_command not in {"file inspect", "file stat", "file copy", "file move", "file delete", "file chmod", "file chown"}:
                schema_command = "file"
            print(dumps_ascii(_json_schema_registry.with_json_schema(schema_command, report or {"command": task, "ok": False, "error": "Unsupported file command"}), indent=2))
            return 0 if report and report.get("ok") else 1
        response = _command_views._handle_file_command(task, config)
    ...
    if (
        ...
        if args.json:
            report = _tool_reports.external_io_report(
                task,
                config,
                execute_external_io_command=_command_dispatch.execute_external_io_command,
            ...
            return 0 if report and report.get("ok") else 1
        response = _command_dispatch.handle_external_io_command(
            task,
            config,
            execute_external_io_command=_command_dispatch.execute_external_io_command,
            record_handoff_action=_project_handoff_views._record_handoff_action,
    ...
    if task == "permissions":
        if args.json:
            print(
                dumps_ascii(
                    _json_schema_registry.with_json_schema("permissions", {"command": "permissions", "report": _status_views._permissions_report(config)}),
                    indent=2,
                )
            )
        else:
            print(_report_views._render_permissions(config))
        return 0
    if task in {"board", "stage review"}:
        if args.json:
            print(dumps_ascii(_json_schema_registry.with_json_schema("board", _report_views._board_report(config)), indent=2))
        else:
            print(_report_views._render_board(config))
        return 0
    if task in {"sessions", "session list"}:
        agent = _agent_setup_views._configure_agent_for_workspace(config)
        if args.json:
            print(dumps_ascii(_json_schema_registry.with_json_schema("sessions", _command_views._shell_sessions_report(agent)), indent=2))
        else:
            shell_session_message = _command_views._handle_shell_session_command(task, agent)
            print(shell_session_message or "No active shell sessions.")
        return 0
    if task.startswith("git "):
        if args.json:
            report = _command_views._git_command_report(task, config)
            schema_command = (report or {}).get("command", "git")
            if schema_command not in {"git status", "git log", "git history", "git show"}:
                schema_command = "git"
            print(dumps_ascii(_json_schema_registry.with_json_schema(schema_command, report or {"command": task, "ok": False, "error": "Unsupported git command"}), indent=2))
            return 0 if report and report.get("ok") else 1
        else:
            git_message = _command_views._handle_git_command(task, config)
    ...
    if task == "boundary":
        if args.json:
            print(dumps_ascii(_json_schema_registry.with_json_schema("boundary", _report_views._boundary_report(config)), indent=2))
        else:
            print(_report_views._render_boundary(config))
        return 0
    if task == "risks" or task.startswith("risks close"):
        if task.startswith("risks close"):
            ...
            if args.json:
                print(dumps_ascii(_json_schema_registry.with_json_schema("risks", _report_views._risks_close_report(config, resolution)), indent=2))
            else:
                print(_report_views._render_risks_close(config, resolution))
            return 0
        if args.json:
            print(dumps_ascii(_json_schema_registry.with_json_schema("risks", _report_views._risks_report(config)), indent=2))
        else:
            print(_report_views._render_risks(config))
        return 0
    if task == "issues" or task.startswith("issues close"):
        if task.startswith("issues close"):
            ...
            if args.json:
                print(dumps_ascii(_json_schema_registry.with_json_schema("issues", _report_views._issues_close_report(config, resolution)), indent=2))
            else:
                print(_report_views._render_issues_close(config, resolution))
            return 0
        if args.json:
            print(dumps_ascii(_json_schema_registry.with_json_schema("issues", _report_views._issues_report(config)), indent=2))
        else:
            print(_report_views._render_issues(config))
        return 0
    if task == "quality" or task.startswith("quality close"):
        if task.startswith("quality close"):
            ...
            if args.json:
                print(dumps_ascii(_json_schema_registry.with_json_schema("quality", _report_views._quality_close_report(config, resolution)), indent=2))
            else:
                print(_report_views._render_quality_close(config, resolution))
            return 0
        if args.json:
            print(dumps_ascii(_json_schema_registry.with_json_schema("quality", _report_views._quality_report(config)), indent=2))
        else:
            print(_report_views._render_quality(config))
        return 0
    if task == "exception":
        if args.json:
            print(dumps_ascii(_json_schema_registry.with_json_schema("exception", _report_views._exception_report(config)), indent=2))
        else:
            print(_report_views._render_exception(config))
        return 0
    if task == "lessons":
        if args.json:
            print(dumps_ascii(_json_schema_registry.with_json_schema("lessons", _report_views._lessons_report(config)), indent=2))
        else:
            print(_report_views._render_lessons(config))
        return 0
    if task == "todo":
        if args.json:
            print(dumps_ascii(_json_schema_registry.with_json_schema("todo", _report_views._todo_report(config)), indent=2))
        else:
            print(_report_views._render_todo(config))
        return 0
    if task in {"transcript", "trace"}:
        if args.json:
            print(dumps_ascii(_json_schema_registry.with_json_schema("transcript", _project_handoff_views._transcript_report(config)), indent=2))
        else:
            print(_project_handoff_views._render_transcript(config))
        return 0
    if task == "handoff":
        if args.json:
            print(dumps_ascii(_json_schema_registry.with_json_schema("handoff", _project_handoff_views._handoff_report(config)), indent=2))
        else:
            print(_project_handoff_views._render_handoff(config))
        return 0
    if task == "handoff actions" or task.startswith("handoff actions "):
        parts = task.split()
        limit = _project_handoff_views._parse_optional_limit(parts)
        if args.json:
            print(dumps_ascii(_json_schema_registry.with_json_schema("handoff", _project_handoff_views._handoff_actions_report(config, limit=limit)), indent=2))
        else:
            print(_project_handoff_views._render_handoff_actions(config, limit=limit))
        return 0
    if task in {"handoff export", "handoff md"}:
        if args.json:
            print(dumps_ascii(_json_schema_registry.with_json_schema("handoff", _project_handoff_views._export_handoff_markdown_report(config)), indent=2))
        else:
            print(_project_handoff_views._export_handoff_markdown(config))
        return 0
    if task == "resume --show":
        if args.json:
            print(dumps_ascii(_json_schema_registry.with_json_schema("resume --show", _project_handoff_views._resume_show_report(config)), indent=2))
        else:
            print(_project_handoff_views._render_resume_show(config))
        return 0
    if task == "resume context":
        if args.json:
            print(dumps_ascii(_json_schema_registry.with_json_schema("resume context", _project_handoff_views._resume_context_payload(config)), indent=2))
        else:
            print(_project_handoff_views._render_resume_context(config))
        return 0
    if task == "resume --clear":
        if args.json:
            print(dumps_ascii(_json_schema_registry.with_json_schema("resume --clear", _project_handoff_views._archive_and_clear_handoff_report(config)), indent=2))
        else:
            print(_project_handoff_views._archive_and_clear_handoff(config))
        return 0
    if task in {"models usage", "cost"}:
        if args.json:
            print(dumps_ascii(_json_schema_registry.with_json_schema("models usage", _status_views._model_usage_report(config)), indent=2))
        else:

...
COMMAND_SPECS: tuple[CommandSpec, ...] = (
    ...
    CommandSpec("goal loop add-node", "core", "Register a custom goal loop node from an extension skill.", "goal loop add-node <name> <skill_path> [purpose]", json=True, handler="status"),
    CommandSpec("goal loop custom-nodes", "core", "List registered custom goal loop nodes.", "goal loop custom-nodes", json=True, handler="status"),
...
HELP_TOPICS: tuple[HelpTopic, ...] = (
    ...
    HelpTopic(
        key="goal loop",
        title="Goal loop orchestration",
        summary="hierarchical multi-node goal loop with TDD, wet-run, and escalation",
        extra_lines=(
            "- goal loop <task>",
            "- goal loop run <task>",
            "- goal loop status",
            "- goal loop add-node <name> <skill_path>",
        ...
        examples=(
            "goal loop Build a Stagewarden multi-node orchestration flow",
            "goal loop run Build a Stagewarden multi-node orchestration flow",
            "help goal loop",
        ),
        aliases=("goal-loop", "orchestration", "multi-node"),
    ),
    HelpTopic(
        key="baseline",

    ...
    path.write_text(
        "\n".join(
            [
                ...
                "        return 0",
                "    step_id = extract(prompt, 'step_id').lower()",
                "    has_step_shell = bool(step_id and re.search(rf'step={re.escape(step_id)} .*action=shell status=ok', prompt_lower))",
                "    needs_tool_evidence = 'wet_run_required=true' in prompt_lower and not has_step_shell",
                "    if needs_tool_evidence and ('support reports active incidents' in instruction or 'close the gate' in instruction or 'close the exception path' in instruction or 'checkpoint recovery' in prompt_lower):",
                '        action = {"type": "shell", "command": "python3 -c \\\"print(\\\'benchmark wet-run validation passed\\\')\\\""}',
                "    elif 'recovery' in prompt_lower or 'exception plan' in prompt_lower or 'board' in prompt_lower:",
